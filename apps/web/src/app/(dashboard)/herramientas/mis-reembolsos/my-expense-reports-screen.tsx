...
import { deleteExpenseReportAction } from './actions';
import { formatDate, money, StatusBadge, STATUS_META } from './expense-shared';
...
export function MyExpenseReportsScreen({ reports, errorKind, activeStatus }: Props) {
  const router = useRouter();
  const [confirming, setConfirming] = useState<ExpenseReportItem | null>(null);
    ...
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-10 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-amber-600">
          No tienes permiso para ver los reportes de reembolso.
        </p>
      </div>
    );
  }

  function runDelete(report: ExpenseReportItem) {
    setError(null);
    startTransition(async () => {
      const res = await deleteExpenseReportAction(report.id);
      if (res.success) {
        setConfirming(null);
        router.refresh();
      } else {
  ...
  function setFilter(value: string) {
    router.push(value ? `/herramientas/mis-reembolsos?status=${value}` : '/herramientas/mis-reembolsos');
  }
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-navy">Mis reembolsos</h1>
        </div>
        <Link
          href="/herramientas/mis-reembolsos/nuevo"
      ...
      {reports.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-200 py-12 text-center text-sm text-slate-400">
          No tienes reportes de reembolso{activeStatus ? ' con ese estado' : ''}.
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-400">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Autorizador</th>
                ...
                {reports.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/60">
                    <td className="px-4 py-3 font-mono text-xs text-slate-500">
                      <Link href={`/herramientas/mis-reembolsos/${r.id}`} className="hover:underline">
                        {r.reportNumber ?? '—'}
                      </Link>
                    </td>
                    ...
                    <td className="max-w-[16rem] truncate px-4 py-3 text-slate-600" title={r.motive}>
                      {r.motive}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        ...
                        {r.status === 'draft' && (
                          <>
                            <Link
                              href={`/herramientas/mis-reembolsos/${r.id}/editar`}
                              className="rounded-md border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:border-slate-300"
                            >
                              Editar
                            </Link>
