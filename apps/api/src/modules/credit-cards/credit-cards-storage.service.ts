...
/** Storage propio del módulo, como el resto (helpdesk, projects, expenses…). */
@Injectable()
export class CreditCardsStorageService {
  ...
  async uploadInvoice(
    ...
    const key = `card-expenses/${reportId}/${lineId}/${Date.now()}_${file.originalname}`;
  ...
  static invoicePrefix(reportId: string, lineId: string): string {
    return `card-expenses/${reportId}/${lineId}`;
  }
  ...
  async getPresignedUploadUrl(
    ...
    const s3Key = buildUploadKey(
      CreditCardsStorageService.invoicePrefix(reportId, lineId),
      fileName,
    );
